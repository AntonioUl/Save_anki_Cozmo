...
cv2.namedWindow("Cozmo's Camera", cv2.WINDOW_AUTOSIZE)
...
class RCApp(object):
    ...
    def init(self):
        ...
        # Initialize Cozmo's camera
        self.cli.add_handler(pycozmo.event.EvtNewRawCameraImage, on_camera_image) #======================================= im here change 1
        self.cli.enable_camera()  # enable camera
    ...
    def term(self):
        ...
        self.cli.disconnect()
        self.cli.stop()
        # Properly close the OpenCV window
    ...
    def run(self):
        ...
        timer = pycozmo.util.FPSTimer(14)  # 14 FPS timer for the robot camera
        
        while not self._stop:
            try:
                if last_im:  # Check if there is a new image received
                    im = last_im
                    im = im.convert('L')  # Convert image to 8-bit pixels, black and white
                    im_np = np.array(im)  # Convert the PIL image to a numpy array
                    cv2.imshow("Cozmo's Camera", im_np)  # Display the image using OpenCV
                    if cv2.waitKey(1) & 0xFF == ord('q'):  # Close on 'q' key
                        break
                timer.sleep()  # Sleep until the next frame
...
def main():
    ...
    app = RCApp("/dev/input/event31") # run evtest in your local temrinal to identify which event is your controller. then change the event number here to suit
